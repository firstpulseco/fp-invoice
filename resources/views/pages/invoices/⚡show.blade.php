...
<div class="invoice-preview-shell bg-ice dark:bg-slate -m-6 min-h-screen p-4 sm:-m-8 sm:p-8 lg:-m-10 lg:p-10">
    ...
    <article class="invoice-sheet text-ink mx-auto min-h-[11in] w-full max-w-[8.5in] bg-white px-[0.55in] py-[0.5in] shadow-xl sm:px-[0.72in] sm:py-[0.65in]">
        ...
        <section class="grid grid-cols-2 gap-x-10 gap-y-12 py-12">
            <div>
                <p class="invoice-kicker">Bill To</p>
                <p class="mt-4 text-xl font-semibold">{{ $invoice->client_company_name }}</p>
                <div class="mt-3 text-sm leading-6 text-zinc-700">
                    ...
                    @endif
                    @if ($invoice->client_email)<p class="mt-2">{{ $invoice->client_email }}</p>@endif
                    @if ($invoice->client_phone)<p>{{ $invoice->client_phone }}</p>@endif
                </div>
            </div>
            <div class="grid grid-cols-2 content-start gap-x-8 gap-y-6">
                <div>
                    <p class="invoice-kicker">Issued</p>
                    <p class="mt-2 text-sm font-medium">{{ $invoice->invoice_date->format('F j, Y') }}</p>
                </div>
                <div>
                    <p class="invoice-kicker">Due</p>
                    <p class="mt-2 text-sm font-medium">{{ $invoice->due_date->format('F j, Y') }}</p>
                </div>
                <div class="col-span-2 border-t border-zinc-300 pt-6">
                    <p class="invoice-kicker">Total Due</p>
                    <p class="mt-2 text-4xl font-medium tracking-[-0.05em] tabular-nums">${{ number_format((float) $invoice->total, 2) }}</p>
                </div>
            </div>
        </section>

        <section>
            <div class="border-ink border-b-2 pb-3">
                <p class="invoice-kicker">Services</p>
            </div>
            <div class="invoice-services">
                <div class="grid grid-cols-[minmax(0,1fr)_5.5rem_4.5rem_6.5rem] gap-4 border-b border-zinc-400 py-2.5 text-[0.62rem] font-semibold text-zinc-600">
            ...
            <div class="border-ink ml-auto grid w-full max-w-[18rem] grid-cols-2 items-baseline border-b-2 py-6">
        ...
        <section class="invoice-payment border-ink mt-14 grid grid-cols-2 gap-10 border-t pt-7">
            <div>
                ...
                <div class="mt-4 grid gap-5">
                    @forelse ($paymentMethods as $index => $paymentMethod)
                        <div wire:key="invoice-payment-method-{{ $index }}">
                            ...
                            @if ($paymentMethod['details'])<p class="text-navy mt-2 whitespace-pre-line text-xs leading-5">{{ $paymentMethod['details'] }}</p>@endif
                        </div>
                    @empty
            ...
            <div>
                ...
                <p class="mt-4 text-sm font-semibold">{{ $invoice->business_name ?: 'Your business' }}</p>
                <div class="mt-2 text-xs leading-5 text-zinc-700">
        ...
        <footer class="invoice-footer mt-14 flex items-end justify-between gap-10 border-t border-zinc-300 pt-5 text-[0.65rem] leading-5 text-zinc-600">
